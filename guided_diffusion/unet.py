...
class UNetModel_MS_Former_MultiStage(nn.Module):
    ...
    def __init__(
        ...
        # Set stage-specific channel counts
        if stage_channels is None:
            # Default: linear decrease in channels
            self.stage_channels = [
                max(model_channels, model_channels // (i+1)) 
        ...
        # Shared input processing for all modalities
        ch = input_ch = int(channel_mult[0] * model_channels)
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(dims, in_channels, ch, 3, padding=1))]
        )
        ...
        # Shared encoder blocks (same as original)
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                layers = [
                    ResBlock(
        ...
        # Shared middle block (same as original)
        self.middle_block = TimestepEmbedSequential(
            ResBlock(
                ch,
                time_embed_dim,
        ...
        # Fusion module (same as original)
        self.fusion = ViT_fusion(
            image_size=(image_size[0]//(2**(len(channel_mult)-1)), 
            image_size[1]//(2**(len(channel_mult)-1))),
            patch_size=(image_size[0]//(2**(len(channel_mult))), 
    ...
    def forward(self, x, timesteps, ct, dis, y=None, stage_indices=None):
        ...
        :param x: Input image [N x C x H x W]
        :param timesteps: Diffusion timesteps [N]
        :param ct: CT scan input [N x CT_C x H x W]
        :param dis: Distance map input [N x DIS_C x H x W]
        :param y: Optional class labels [N]
        :param stage_indices: Optional stage indices [N]
        ...
        # If stage_indices not provided, determine from timesteps
        ...
        # Process all modalities through shared encoder
        h = x.type(self.dtype)
    ...
    def get_stage_from_timestep(self, timesteps, num_timesteps=1000):
        """
        Map timesteps to stage indices (0=high noise, num_stages-1=low noise)
        """
